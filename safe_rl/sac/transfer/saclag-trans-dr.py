...
from safe_rl.utils.load_utils_transfer import load_policy_transfer
...
class ReplayBuffer:
    ...
    def store(self, obs, act, rew, rew1, next_obs, done, cost):
...
def sac(env_fn, get_logp_a_fn, get_teacher_a_fn, teacher_size, teacher_keys, actor_fn=mlp_actor, critic_fn=mlp_critic,  ac_kwargs=dict(), seed=0,
        ...
        reward_scale=1, lr_scale = 1, damp_scale = 0, trans_bonus = 1
        ):
    """
    Args:
    ...
    #_, get_logp_a, _ = load_policy_transfer(fpath, itr)
    ...
    x_ph, a_ph, x2_ph, r_ph, r1_ph, d_ph, c_ph = placeholders(obs_dim, act_dim, obs_dim, None, None, None, None)
    tb_ph = tf.placeholder(tf.float32)
    ...
    '''
    '''
    q_backup = tf.stop_gradient(r_ph + tb_ph*r1_ph + gamma*(1-d_ph)*(min_q_pi_targ - alpha * logp_pi2))
    ...
    decay_rate = trans_bonus/(epochs-10)
    number_model = 0
    cum_cost = 0
    local_steps = 0
    ...
    for t in range(total_steps // num_procs()):
        ...
        #TODO: Mix the teacher policy and the student policy
        '''
        if np.random.uniform() > trans_bonus:
            a = get_action(o)
        else:
        ...
        #bonus to encourage the teacher policy
        #r += 0.5*log_p_a
        ...
        replay_buffer.store(o, a, r, r1, o2, d, c)
        ...
        if t > 0 and t % update_freq == 0:
            ...
            for j in range(update_freq):
                ...
                #log_p_a = get_logp_a_fn(batch['obs1'], batch['acts'])
        ...
        if t > 0 and t % local_steps_per_epoch == 0:
            ...
            #gradually decrease the trans_bonus
            trans_bonus -= decay_rate
            if trans_bonus <= 0.02:
                trans_bonus = 0.02
            ...
            logger.log_tabular('TransBonus', trans_bonus)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='StaticEnv-v0')
    ...
    parser.add_argument('--cost_lim', type=float, default=15)
    parser.add_argument('--lr_s', type=int, default=1)
    parser.add_argument('--damp_s', type=int, default=0)
    parser.add_argument('--trans_bonus', type=float, default=1)
    ...
    sac(lambda: gym.make(args.env), get_logp_a_fn = get_logp_a, get_teacher_a_fn = get_teacher_a, teacher_size = _teacher_size, teacher_keys = _teacher_keys, actor_fn=mlp_actor, critic_fn=mlp_critic, 
        ...
        fixed_cost_penalty=args.fixed_cost_penalty, cost_constraint=args.cost_constraint, cost_lim = args.cost_lim, lr_scale = args.lr_s, damp_scale = args.damp_s, trans_bonus = args.trans_bonus
        )