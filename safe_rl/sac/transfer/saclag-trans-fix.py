...
Soft Actor-Critic
"""
def sac(env_fn, get_logp_a_fn, teacher_size, teacher_keys, actor_fn=mlp_actor, critic_fn=mlp_critic,  ac_kwargs=dict(), seed=0,
    ...
    for t in range(total_steps // num_procs()):
        ...
        log_p_a = get_logp_a_fn(o_teacher, a)
...
if __name__ == '__main__':
    ...
    teacher_env, get_logp_a, _, _ = load_policy_transfer(args.fpath, args.itr if args.itr >= 0 else 'last')
    
    _teacher_size = teacher_env.obs_flat_size
    _teacher_keys = teacher_env.obs_space_dict.keys()

    sac(lambda: gym.make(args.env), get_logp_a_fn = get_logp_a, teacher_size = _teacher_size, teacher_keys = _teacher_keys, actor_fn=mlp_actor, critic_fn=mlp_critic, 
